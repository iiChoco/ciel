...
class MlxWhisperSTT:
    ...
    async def transcribe(self, pcm: np.ndarray) -> str:
        ...
        # GPU and make both slower than running them back to back. The lock
        # belongs to the worker thread, not to this coroutine, so a caller
        # cancelled mid-decode cannot let the next one in beside it.
        started = time.monotonic()
