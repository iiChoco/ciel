...
async def probe_door(state: Path) -> None:
    ...
    async with TestClient(TestServer(app)) as client:
        ...
        r = await client.get("/interview/mark.js")
        mark = await r.text()
        check("the living mark is served from the Chart's own engine", r.status == 200 and "class CielSymbol2" in mark and "root.CielSymbol2 = CielSymbol2" in mark)
        check("the mark script is script alone, with no markup around it", "<script" not in mark and "</script" not in mark)
        page = await (await client.get("/interview")).text()
        check("every mark on the page keeps a still beside its canvas", page.count('class="mark-still"') == page.count('class="mark-live"') == 3 and "aperture" not in page)
